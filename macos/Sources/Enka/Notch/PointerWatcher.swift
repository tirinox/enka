...
final class PointerWatcher {
    ...
    /// things too — the status menu, a tab that wants the keyboard.
    ...
    /// the panel is open — with hysteresis. Motion: the pointer must have moved
    /// recently, because a parked pointer is not on its way anywhere, however
    /// close to the notch it is parked. The menu bar, where cursors spend half
    /// their lives, lies entirely inside the band.
    ...
    /// Force the state, e.g. when the panel is toggled from the status menu.
    ...
    private func tick() {
        ...
        let interactive = interactiveRect.contains(point)
        ...
        // agree, but every path that opens the panel without the pointer — the
        // status menu, a tab claiming the keyboard — is a chance for them to
        // come apart, and the shape that costs the user something is always the
        // same: a panel standing open with the mouse nowhere near it, waiting
        // for a hover that already happened. Whatever led there, the pointer is
        // the authority, so say so again.
        if !inside, !isInside, isPanelOpen() {
