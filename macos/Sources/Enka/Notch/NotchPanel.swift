...
///
/// Adapted from Cyclop (MIT, github.com/akalikbergenov/cyclop).
final class NotchPanel: NSPanel {
    ...
    /// far too rude for a panel one merely hovers. The tabs that type — and
    /// the study loop, once it has been clicked into — turn it on for as long
    /// as they are in use.
    ///
    ...
    /// which is why matching on characters quietly fails exactly for the person
    /// typing Russian. Key codes name the key, not what it prints.
    ///
    /// Enka's own study shortcuts are matched the same way and for a sharper
    /// reason: this is a language app, so its user is *expected* to be sitting
    /// on a foreign layout while studying.
    private enum Key {
    ...
    ///
    /// A tap gesture on a text editor never fires — the text view claims the
    /// mouse down first — so the one place a click on a field can reliably be
    /// noticed is here, where every event the window receives passes.
    var onPress: (() -> Void)?

    /// Consulted on every key press the panel receives, before the press is
    /// delivered. Returns true when it has dealt with the key.
    ///
    /// This is where the study shortcuts live, and they live here rather than
    /// in a SwiftUI `onKeyPress` for one reason: `KeyPress` reports the
    /// character produced, and Enka is a language app. Its user is expected to
    /// be sitting on a foreign layout — that is the whole point of the
    /// collection — so "u" for undo is "г" half the time. A key code names the
    /// key regardless of what it prints.
    ///
    /// The handler is only asked while the tab showing has no field in it; the
    /// controller enforces that, so a digit typed into the add tab stays a
    /// digit rather than becoming a rating.
    ...
    override func sendEvent(_ event: NSEvent) {
        ...
        // reaches a field and places a caret.
    ...
    init(contentRect: NSRect) {
        ...
        // not — a `TextField` asks the effective appearance what colour text is,
        // and on a Mac set to light it is told black. The result is a field one
        // can type into and not read.
