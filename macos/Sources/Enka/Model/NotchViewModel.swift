...
final class NotchViewModel: ObservableObject {
    enum Tab: String, CaseIterable, Identifiable {
        case study, add, search, stats, settings
        ...
        var needsKeyboard: Bool { self == .add || self == .search || self == .settings }

        /// The tabs that show a list, and get the taller body for it.
        var isTall: Bool { self == .search || self == .stats }
    }
    ...
    init(geometry: NotchGeometry, session: Session, audio: AudioPlayback, stats: StatsStore) {
        ...
        for child in [session.objectWillChange, study.objectWillChange, stats.objectWillChange] {
            child
                .sink { [weak self] _ in
